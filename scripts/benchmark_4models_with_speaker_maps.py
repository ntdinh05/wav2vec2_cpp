#!/usr/bin/env python3
"""4-model benchmark with speaker_map.json-based splits.

Ensures consistent test splits across all models using JSON speaker maps.
This addresses potential data leakage by explicitly documenting which
speakers are used for training vs testing.
...
Output (in output/):
  ...
  benchmark_splits_used.json          — Speaker maps used for this benchmark
"""
...
# Speaker maps (with fallback to original method)
UXTD_SPEAKER_MAP_PATH  = os.path.join(PROJECT_DIR, "output", "uxtd_speaker_map.json")
TAL80_SPEAKER_MAP_PATH = os.path.join(PROJECT_DIR, "output", "speaker_map.json")
...
# ── Dataset helpers ───────────────────────────────────────────────────
def load_uxtd_test():
    """Load UXTD test split using speaker_map.json if available."""
    ...
    # Try to use speaker_map.json
    if os.path.exists(UXTD_SPEAKER_MAP_PATH):
        with open(UXTD_SPEAKER_MAP_PATH) as f:
            speaker_map = json.load(f)
        test_speakers = speaker_map.get("test", [])
        print(f"  Using speaker_map.json for UXTD ({len(test_speakers)} test speakers)")
    else:
        ...
        test_speakers = sorted(df[df["split"] == "test"]["speaker"].unique().tolist())
        print(f"  Using speakers file for UXTD ({len(test_speakers)} test speakers)")

    df = df[df["speaker"].isin(test_speakers)].reset_index(drop=True)
    return df, sorted(test_speakers)


def load_tal80_test():
    """Load TaL80 test split using speaker_map.json."""
    df = pd.read_csv(TAL80_CSV)
    df["wav_path"] = df["filepath"].str.replace(r"\.txt$", ".wav", regex=True)
    df = df[df["wav_path"].apply(os.path.exists)].reset_index(drop=True)

    with open(TAL80_SPEAKER_MAP_PATH) as f:
        speaker_map = json.load(f)
    test_speakers = sorted(speaker_map["test"])
    print(f"  Using speaker_map.json for TaL80 ({len(test_speakers)} test speakers)")
...
def main():
    ...
    # Document which speaker maps were used
    splits_used = {
        "uxtd": {
            "test_speakers": uxtd_speakers,
            "num_speakers": len(uxtd_speakers),
            "num_utterances": len(uxtd_df),
        },
        "tal80": {
            "test_speakers": tal80_speakers,
            "num_speakers": len(tal80_speakers),
            "num_utterances": len(tal80_df),
        },
    }
    ...
    # ── Save speaker maps used ─────────────────────────────────────────
    splits_path = os.path.join(OUTPUT_DIR, "benchmark_splits_used.json")
    with open(splits_path, "w") as f:
        json.dump(splits_used, f, indent=2)
    print(f"Splits documentation saved: {splits_path}")
